...
func score_updated():
	timer.wait_time -= 0.05 # makes the enemies spawn faster
	print("score from hud:", score)
	if score >= 1000 && !Globals.boss_fight_started:
		var boss = load("res://scenes/boss.tscn").instantiate()
		enemy_container.add_child(boss)
		Globals._on_bossfight_started()

# New method to safely change scenes
func change_to_end_cutscene():
